...
def find_bit_integral_distinguisher(block_size, rounds, cipher_name, CIPHER_CLASS_NAME):
    ...
    for active_point in range(1):
        ...
        filename_model = 'lp/' + cipher_name + '_R%i_A%i_model.lp' % (rounds, active_point)
        filename_result = 'result/' +cipher_name + '_R%i_A%i_result.txt' % (rounds, active_point)
    ...
    #
    # filename_result = 'result_statistics/---'+ cipher_name +'----R%i_AllResult.txt' % (rounds)
    # file_r = open(filename_result, "w+")
    # for i in len_zero:
    #     file_r.write(i)
    #     file_r.write('\n')
